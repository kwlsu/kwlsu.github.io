 ...
 *       subtitle: " <b>KIWOLASU LOCAL VERSION</b> · 3.2",
...
(function (root, factory) {
    ...
    ".kbfab-wrap{position:fixed;left:5vw;top:10vh;z-index:2147483000;touch-action:none;font-family:Arial,sans-serif;}" +
    ...
    ".kbfab-btn{position:relative;width:56px;height:56px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:rgba(var(--kbfab-bg-rgb),var(--kbfab-bg-alpha));border:1px solid rgba(80,195,255,.55);box-shadow:none;cursor:pointer;transition:transform .18s ease,background .18s ease,border-color .18s ease;font-size:24px;color:#8de8ff;user-select:none;}" +
    ".kbfab-btn:active{transform:scale(.92);}" +
    ".kbfab-btn.kbfab-open{border-color:rgba(130,230,255,.9);}" +
    ".kbfab-wrap.kbfab-dragging .kbfab-btn{transform:scale(1.06);}" +
    ".kbfab-btn img{width:48px;height:48px;object-fit:contain;pointer-events:none;transition:transform .25s ease;}" +
    ...
    function KBFabInstance(opts) {
        ...
        var eyebrowHtml = '<span class="kbfab-dot"></span> <span class="kbfab-clock"></span>';
        var eyebrow = el("div", "kbfab-eyebrow", eyebrowHtml);
        var clockEl = eyebrow.querySelector(".kbfab-clock");
        ...
        function updateClock() {
            ...
            var date = pad2(now.getDate()) + " " + MONTHS_ID[now.getMonth()] + " " + now.getFullYear();
        ...
        // ---- klik tracker module (catat klik ke Firebase, dikelompokkan per menu induk) ----
        var klikApi = null;
        if (opts.klik && opts.klik.baseUrl) {
            klikApi = buildKlik(opts.klik);
        }

        function buildKlik(kOpts) {
            var KLIK_URL = kOpts.baseUrl + ".json";

            function catat(menuId) {
                if (!menuId) return;
                try {
                    fetch(KLIK_URL, {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ menu: menuId, waktu: Date.now() })
                    }).catch(function () {});
                } catch (_) {}
            }
    ...
    var DEFAULT_CONFIG = {
        ...
        klik: {
            baseUrl: "https://pucuk-9d1d6-default-rtdb.asia-southeast1.firebasedatabase.app/links/klik_log"
        }
    };
